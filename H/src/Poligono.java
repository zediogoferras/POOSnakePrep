...
public class Poligono {
    ...
    public Ponto findCentroid(){
        ...
        for(int i = 0; i < this.pontos.length-1; i++){
            x0 = this.pontos[i].x();
            y0 = this.pontos[i].y();
            x1 = this.pontos[i+1].x();
            y1 = this.pontos[i+1].y();
            a = x0*y1 - x1*y0;
            sa += a;
            xC += (x0+x1)*a;
            yC += (y0+y1)*a;
        }
        x0 = this.pontos[this.pontos.length-1].x();
        y0 = this.pontos[this.pontos.length-1].y();
        x1 = this.pontos[0].x();
        y1 = this.pontos[0].y();
        a = x0*y1 - x1*y0;
        sa += a;
        ...
        return new Ponto((int) round(xC), (int) round(yC));
    }
     ...
     */
    public Poligono rotate(int angle, Ponto c){
        double x0, y0, x1, y1;
        ...
        for(int i = 0; i < this.pontos.length; i++){
            ...
            rotatedPoints[i] = new Ponto((int) round(x1), (int) round(y1));
        }
        return new Poligono(rotatedPoints);
    }
